...
| Primary market(s) | Egypt (and possibly wider MENA — confirm). |
...
Unconfirmed starting point: likely EN + AR (Arabic is RTL). Confirm the full set.
...
- **Vertical(s):** Automotive marketplace (new and used cars, and related). Motors-specific vocabulary and flows — confirm the exact verticals (cars only, or bikes/parts too).
...
- **Anything structurally unusual:** Vehicle listings carry make/model/year/mileage attributes; the contact/lead model may differ from property and general-classifieds apps.

**Cross-posting relationship with Dubizzle Egypt:** Dubizzle EG can cross-post a Cars-for-Sale ad to the Hatla2ee platform when an eligible agency enters a Hatla2ee phone/WhatsApp number, and cross-posted ads carry Hatla2ee branding on the Dubizzle DPV. That integration is documented in `dubizzle-egypt.md` Section F. When designing for the standalone Hatla2ee app here, confirm how a cross-posted ad appears **on Hatla2ee's own side** — it is a likely regression seam.

**Note for whoever fills this in:** Automotive-specific: listings are vehicles with make/model/year/mileage attributes, and the lead/contact model may differ from the property and general-classifieds apps. Capture the actual vocabulary and lead types — do not assume Bayut or Dubizzle patterns transfer.
