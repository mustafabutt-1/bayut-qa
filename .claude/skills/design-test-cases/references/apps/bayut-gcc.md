...
| Primary market(s) | GCC markets outside UAE/KSA — Oman and others. **Confirm exactly which countries this app covers.** |
...
Unconfirmed starting point: likely EN + AR (Arabic is RTL). Confirm per market — the set may vary by country.
...
- **Vertical(s):** Residential and commercial property, same product family as Bayut UAE.
...
**Note for whoever fills this in:** A multi-market app: confirm whether language set, currency, and available surfaces vary by country, and whether the app routes markets internally. Each such variation is a test dimension.
