...
Unconfirmed starting point: likely EN + AR (Arabic is RTL). Do **not** assume ZH/RU parity with Bayut UAE — confirm the actual set.
...
- **Vertical(s):** Residential and commercial property (Sale and Rent), same product family as Bayut UAE.
...
**Note for whoever fills this in:** Likely shares the DPV/LPV/MapView/Purpose vocabulary and lead model with Bayut UAE, but the slug and filter configuration has historically differed. Confirm both, and note any KSA-only surfaces.
